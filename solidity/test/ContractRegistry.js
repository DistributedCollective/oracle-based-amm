...
contract("ContractRegistry", (accounts) => {
	...
	it("should revert when attempting to register the registry to the zero address", async () => {
		await expectRevert(contractRegistry.registerAddress(registry.CONTRACT_REGISTRY, ZERO_ADDRESS), "ERR_INVALID_ADDRESS");
	});

	it("should revert when a non owner attempts to register a contract address", async () => {
		await expectRevert(contractRegistry.registerAddress(contractName1bytes, address1, { from: nonOwner }), "ERR_ACCESS_DENIED");
	});
	...
	it("verifies that the deprecated function getAddress works correctly", async () => {
		...
		expect(await contractRegistry.getAddress.call(contractName1bytes)).to.eql(await contractRegistry.addressOf.call(contractName1bytes));
		expect(await contractRegistry.getAddress.call(contractName2bytes)).to.eql(await contractRegistry.addressOf.call(contractName2bytes));
		expect(await contractRegistry.getAddress.call(contractName3bytes)).to.eql(await contractRegistry.addressOf.call(contractName3bytes));
	});
});
