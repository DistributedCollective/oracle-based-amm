...
contract("PriceOracle", (accounts) => {
	...
	it("should revert when attempting to construct a price oracle with zero token A address", async () => {
		await expectRevert(PriceOracle.new(ZERO_ADDRESS, tokenB.address, chainlinkOracleA.address, chainlinkOracleB.address), "ERR_INVALID_ADDRESS");
	});

	it("should revert when attempting to construct a price oracle with zero token B address", async () => {
		await expectRevert(PriceOracle.new(tokenA.address, ZERO_ADDRESS, chainlinkOracleA.address, chainlinkOracleB.address), "ERR_INVALID_ADDRESS");
	});
	...
	it("should revert when attempting to construct a price oracle with same tokens", async () => {
		...
		await chainlinkOracleA.setTimestamp(5000);
		await chainlinkOracleB.setTimestamp(5010);
		await expectRevert(PriceOracle.new(tokenA.address, tokenA.address, chainlinkOracleA.address, chainlinkOracleB.address), "ERR_SAME_ADDRESS");
	});

	it("should revert when attempting to construct a price oracle with zero chainlink oracle A address", async () => {
		await expectRevert(PriceOracle.new(tokenA.address, tokenB.address, ZERO_ADDRESS, chainlinkOracleB.address), "ERR_INVALID_ADDRESS");
	});

	it("should revert when attempting to construct a price oracle with zero chainlink oracle B address", async () => {
		await expectRevert(PriceOracle.new(tokenA.address, tokenB.address, chainlinkOracleA.address, ZERO_ADDRESS), "ERR_INVALID_ADDRESS");
	});
	...
	it("should revert when attempting to construct a price oracle with same chainlink oracles", async () => {
		...
		await chainlinkOracleA.setAnswer(10000);
		await chainlinkOracleA.setTimestamp(5000);
		await expectRevert(PriceOracle.new(tokenA.address, tokenB.address, chainlinkOracleA.address, chainlinkOracleA.address), "ERR_SAME_ADDRESS");
	});

	it("should revert when attempting to construct a price oracle with a non-standard token A", async () => {
		await expectRevert.unspecified(PriceOracle.new(accounts[5], tokenB.address, chainlinkOracleA.address, chainlinkOracleB.address));

		const token = await TestNonStandardTokenWithoutDecimals.new("ERC Token 3", "ERC3", 10000000000);
		await expectRevert.unspecified(PriceOracle.new(token.address, tokenB.address, chainlinkOracleA.address, chainlinkOracleB.address));
	});

	it("should revert when attempting to construct a price oracle with a non-standard token B", async () => {
		await expectRevert.unspecified(PriceOracle.new(tokenA.address, accounts[5], chainlinkOracleA.address, chainlinkOracleB.address));

		const token = await TestNonStandardTokenWithoutDecimals.new("ERC Token 3", "ERC3", 10000000000);
		await expectRevert.unspecified(PriceOracle.new(tokenA.address, token.address, chainlinkOracleA.address, chainlinkOracleB.address));
	});
