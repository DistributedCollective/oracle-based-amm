...
contract("LiquidTokenConverter", (accounts) => {
	...
	const convert = async (path, amount, minReturn, options = {}) => {
		return sovrynSwapNetwork.convertByPath(path, amount, minReturn, ZERO_ADDRESS, ZERO_ADDRESS, 0, { from: sender, ...options });
	};

	const convertCall = async (path, amount, minReturn, options = {}) => {
		return sovrynSwapNetwork.convertByPath.call(path, amount, minReturn, ZERO_ADDRESS, ZERO_ADDRESS, 0, { from: sender, ...options });
	};
	...
	for (let isETHReserve = 0; isETHReserve < 2; isETHReserve++) {
		describe(`${isETHReserve === 0 ? "(with ERC20 reserve)" : "(with ETH reserve)"}:`, () => {
			...
			it("verifies that selling right after buying does not result in an amount greater than the original purchase amount", async () => {
				...
				const purchaseAmount = await convertCall([getReserve1Address(isETHReserve), tokenAddress, tokenAddress], amount, MIN_RETURN, {
					value,
				});
			...
			it("verifies that buying right after selling does not result in an amount greater than the original sale amount", async () => {
				...
				const purchaseAmount = await convertCall([getReserve1Address(isETHReserve), tokenAddress, tokenAddress], saleAmount, MIN_RETURN, {
					value,
				});
			...
			it("should revert when attempting to convert with 0 minimum requested amount", async () => {
				...
				await expectRevert(convert([getReserve1Address(isETHReserve), tokenAddress, tokenAddress], amount, 0, { value }), "ERR_ZERO_VALUE");
			});
			...
			it("should revert when attempting to buy while the converter is not active", async () => {
				...
				await expectRevert.unspecified(
					convert([getReserve1Address(isETHReserve), tokenAddress, tokenAddress], amount, MIN_RETURN, { value })
				);
			});
			...
			it("should revert when attempting to buy with a non reserve address", async () => {
				...
				await expectRevert(convert([erc20Token.address, tokenAddress, tokenAddress], amount, MIN_RETURN, { value }), "ERR_INVALID_TOKEN");
			});
			...
			it("should revert when attempting to sell with a non reserve address", async () => {
				...
				await expectRevert(convert([erc20Token.address, tokenAddress, tokenAddress], amount, MIN_RETURN), "ERR_INVALID_TOKEN");
			});
			...
			it("verifies that convert is allowed for a whitelisted account", async () => {
				...
				await convert([getReserve1Address(isETHReserve), tokenAddress, tokenAddress], amount, MIN_RETURN, { from: whitelisted, value });
			});
			...
			it("should revert when calling convert from a non whitelisted account", async () => {
				...
				await expectRevert(
					convert([getReserve1Address(isETHReserve), tokenAddress, tokenAddress], amount, MIN_RETURN, { from: whitelisted, value }),
					"ERR_NOT_WHITELISTED"
				);
			});
			...
			it("verifies that targetAmountAndFee returns the same amount as converting", async () => {
				...
				const returnAmount2 = await convertCall([getReserve1Address(isETHReserve), tokenAddress, tokenAddress], amount, MIN_RETURN, {
					value,
				});
