...
contract("ConverterRegistry", (accounts) => {
	...
	describe("add converters", () => {
		...
		context("with registered converters", async () => {
			...
			it("should find liquidity pool by its configuration", async () => {
				...
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchor4.address], [0x2400, 0x2100])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchor6.address], [0x3600, 0x3100])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchorA.address], [0x5a00, 0x5100])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchor8.address], [0x4800, 0x4100])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchorC.address], [0x6c00, 0x6100])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor2.address, anchorE.address], [0x7e00, 0x7200])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor4.address, anchor1.address], [0x2100, 0x2400])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor6.address, anchor1.address], [0x3100, 0x3600])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor8.address, anchor1.address], [0x4100, 0x4800])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchorA.address, anchor1.address], [0x5100, 0x5a00])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchorC.address, anchor1.address], [0x6100, 0x6c00])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchorE.address, anchor2.address], [0x7200, 0x7e00])).to.eql(
					ZERO_ADDRESS
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchor4.address], [0x2100, 0x2400])).to.eql(
					anchor2.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchor6.address], [0x3100, 0x3600])).to.eql(
					anchor3.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchor8.address], [0x4100, 0x4800])).to.eql(
					anchor4.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchorA.address], [0x5100, 0x5a00])).to.eql(
					anchor5.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor1.address, anchorC.address], [0x6100, 0x6c00])).to.eql(
					anchor6.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor2.address, anchorE.address], [0x7200, 0x7e00])).to.eql(
					anchor7.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor4.address, anchor1.address], [0x2400, 0x2100])).to.eql(
					anchor2.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor6.address, anchor1.address], [0x3600, 0x3100])).to.eql(
					anchor3.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchor8.address, anchor1.address], [0x4800, 0x4100])).to.eql(
					anchor4.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchorA.address, anchor1.address], [0x5a00, 0x5100])).to.eql(
					anchor5.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchorC.address, anchor1.address], [0x6c00, 0x6100])).to.eql(
					anchor6.address
				);
				expect(await converterRegistry.getLiquidityPoolByReserveConfig.call([anchorE.address, anchor2.address], [0x7e00, 0x7200])).to.eql(
					anchor7.address
				);
			});
	...
	describe("create converters", () => {
		const testCreate = async (type, name, symbol, decimals, maxConversionFee, reserveTokens, reserveWeights) => {
			await converterRegistry.newConverter(type, name, symbol, decimals, maxConversionFee, reserveTokens, reserveWeights);
			const converter = await ConverterBase.at(await converterRegistry.createdConverter.call());

			const res = await converterRegistry.setupConverter(type, reserveTokens, reserveWeights, converter.address);
		...
		it("should fail to create and setup converter from different addresses", async () => {
			...
			const converter = await ConverterBase.at(await converterRegistry.createdConverter.call());
			await expectRevert(
				converterRegistry.setupConverter(2, [erc20Token2.address, ETH_RESERVE_ADDRESS], [0x6200, 0x6000], converter.address, {
					from: accounts[1],
				}),
				"only the deployer may finish the converter setup"
			);
		});
		...
		context("with created converters", async () => {
			...
			it("should find liquidity pool by its configuration", async () => {
				...
				expect(
					await converterRegistry.getLiquidityPoolByReserveConfig.call([ETH_RESERVE_ADDRESS, erc20Token1.address], [0x4000, 0x4100])
				).to.eql(anchors[3]);
				expect(
					await converterRegistry.getLiquidityPoolByReserveConfig.call([erc20Token1.address, erc20Token2.address], [0x5100, 0x5200])
				).to.eql(anchors[4]);
				expect(
					await converterRegistry.getLiquidityPoolByReserveConfig.call([erc20Token2.address, ETH_RESERVE_ADDRESS], [0x6200, 0x6000])
				).to.eql(anchors[5]);
			});
