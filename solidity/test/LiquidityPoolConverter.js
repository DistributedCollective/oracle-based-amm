...
contract("LiquidityPoolConverter", (accounts) => {
	...
	const createAnchor = async (type) => {
		switch (type) {
			case 1:
				return await SmartToken.new("Pool1", "POOL1", 2);
			case 2:
		...
		for (let isETHReserve = 0; isETHReserve < 2; isETHReserve++) {
			describe(`${getConverterName(type)}${isETHReserve === 0 ? "(with ERC20 reserves)" : "(with ETH reserve)"}:`, () => {
				...
				it("should revert when attempting to add a reserve that already exists", async () => {
					...
					await expectRevert(converter.addReserve(getReserve1Address(isETHReserve), WEIGHT_20_PERCENT), "ERR_INVALID_RESERVE");
				});
				...
				it("should revert when attempting to add multiple reserves with total weight greater than 100%", async () => {
					...
					await expectRevert(converter.addReserve(reserveToken2.address, WEIGHT_50_PERCENT.add(new BN(1))), "ERR_INVALID_RESERVE_WEIGHT");
				});
				...
				it("verifies that targetAmountAndFee returns a valid amount", async () => {
					...
					const returnAmount = (
						await converter.targetAmountAndFee.call(getReserve1Address(isETHReserve), reserveToken2.address, amount)
					)[0];
				...
				it("should revert when attempting to convert with 0 minimum requested amount", async () => {
					...
					await expectRevert(
						convert([getReserve1Address(isETHReserve), anchorAddress, reserveToken2.address], amount, 0, { value }),
						"ERR_ZERO_VALUE"
					);
				});
				...
				it("should revert when attempting to convert when the return is smaller than the minimum requested amount", async () => {
					...
					await expectRevert(
						convert([getReserve1Address(isETHReserve), anchorAddress, reserveToken2.address], amount, 2000, { value }),
						"ERR_RETURN_TOO_LOW"
					);
				});
				...
				it("should revert when calling convert from a non whitelisted account", async () => {
					...
					await expectRevert(
						convert([getReserve1Address(isETHReserve), anchorAddress, reserveToken2.address], amount, MIN_RETURN, {
							from: whitelisted,
							value,
						}),
						"ERR_NOT_WHITELISTED"
