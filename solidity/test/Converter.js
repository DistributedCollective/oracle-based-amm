		...
		for (let isETHReserve = 0; isETHReserve < 2; isETHReserve++) {
			describe(`${getConverterName(type)}${isETHReserve === 0 ? "" : " (with ETH reserve)"}:`, () => {
				...
				it("should revert when attempting to construct a converter with invalid conversion fee", async () => {
					await expectRevert(createConverter(type, anchorAddress, contractRegistry.address, 1000001), "ERR_INVALID_CONVERSION_FEE");
				});
				...
				it("should revert when the owner attempts to withdraw other tokens from the anchor while the converter is not active", async () => {
					...
					await expectRevert(converter.withdrawFromAnchor(ercToken.address, owner, value), "ERR_ACCESS_DENIED");
				});
				...
				it("should revert when a non owner attempts to withdraw other tokens from the anchor", async () => {
					...
					await expectRevert(converter.withdrawFromAnchor(ercToken.address, owner, value, { from: nonOwner }), "ERR_ACCESS_DENIED");
				});
				...
				it("should revert when a non owner attempts update the conversion whitelist contract address", async () => {
					...
					await expectRevert(converter.setConversionWhitelist(receiver, { from: nonOwner }), "ERR_ACCESS_DENIED");
				});
				...
				it("should revert when attempting to update the fee to an invalid value", async () => {
					...
					await expectRevert(converter.setConversionFee(MAX_CONVERSION_FEE.add(new BN(1))), "ERR_INVALID_CONVERSION_FEE");
				});
				...
				it("should revert when attempting to add a reserve with weight = 0", async () => {
					...
					await expectRevert(converter.addReserve(getReserve1Address(isETHReserve), 0), "ERR_INVALID_RESERVE_WEIGHT");
				});
				...
				it("should revert when attempting to add a reserve with weight greater than 100%", async () => {
					...
					await expectRevert(converter.addReserve(getReserve1Address(isETHReserve), 1000001), "ERR_INVALID_RESERVE_WEIGHT");
				});
				...
				it("should revert when a non owner attempts to transfer the anchor ownership", async () => {
					...
					await expectRevert(converter.transferAnchorOwnership(nonOwner, { from: nonOwner }), "ERR_ACCESS_DENIED");
				});
				...
				it("should revert when the owner attempts to withdraw a reserve token while the converter is active", async () => {
					...
					if (isETHReserve) {
						...
						await expectRevert(converter.withdrawTokens(getReserve1Address(isETHReserve), receiver, value), "ERR_ACCESS_DENIED");
					}
				});
				...
				it("should revert when a non owner attempts to withdraw a non reserve token while the converter is not active", async () => {
					...
					await expectRevert(converter.withdrawTokens(token.address, receiver, value2, { from: nonOwner }), "ERR_ACCESS_DENIED");
				});
				...
				it("should revert when a non owner attempts to withdraw a reserve token while the converter is not active", async () => {
					...
					if (isETHReserve) {
						...
						await expectRevert(
							converter.withdrawTokens(getReserve1Address(isETHReserve), receiver, value, { from: nonOwner }),
							"ERR_ACCESS_DENIED"
						);
					}
				});
				...
				it("should revert when attempting to get the target amount with identical source/target addresses", async () => {
					...
					await expectRevert(
						converter.targetAmountAndFee.call(getReserve1Address(isETHReserve), getReserve1Address(isETHReserve), 500),
				...
				it("should revert when attempting to convert with an invalid target token address", async () => {
					...
					await expectRevert(
						convert([getReserve1Address(isETHReserve), anchorAddress, ZERO_ADDRESS], amount, MIN_RETURN, { value }),
						type === 0 ? "ERR_INVALID_TOKEN" : "ERR_INVALID_RESERVE"
					);
				});
				...
				it("should revert when attempting to convert with identical source/target addresses", async () => {
					...
					await expectRevert(
						convert([getReserve1Address(isETHReserve), anchorAddress, getReserve1Address(isETHReserve)], amount, MIN_RETURN, { value }),
