...
contract("ERC20Token", (accounts) => {
	...
	it("should revert when attempting to transfer from another account more than the allowance", async () => {
		...
		await expectRevert(token.transferFrom(sender, receiver2, value.add(new BN(1)), { from: receiver }), "ERR_UNDERFLOW");
	});
	...
	it("should revert when attempting to transfer from an invalid account", async () => {
		...
		await expectRevert(token.transferFrom(ZERO_ADDRESS, receiver2, new BN(0), { from: receiver }), "ERR_INVALID_ADDRESS");
	});
	...
	it("should revert when attempting to transfer from to an invalid account", async () => {
		...
		await expectRevert(token.transferFrom(sender, ZERO_ADDRESS, new BN(0), { from: receiver }), "ERR_INVALID_ADDRESS");
	});
});
