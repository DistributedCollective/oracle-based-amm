...
contract("ConverterUpgrader", (accounts) => {
	...
	const initWithoutReserves = async (deployer, version, activate) => {
		const smartToken = await SmartToken.new("Smart Token", "TKN1", 0);
		const converter = await ConverterHelper.new(0, smartToken.address, contractRegistry.address, MAX_CONVERSION_FEE, ZERO_ADDRESS, 0, version);
		const upgrader = await ConverterUpgrader.new(contractRegistry.address, ZERO_ADDRESS);
	...
	const upgradeConverter = async (upgrader, converter, options = {}) => {
		...
		if (converter.upgrade) {
			...
			res = await upgrader.upgradeOld(converter.address, web3.utils.asciiToHex(""), { from: accounts[0], ...options });
	...
	const product = cartesian(
		[initWithoutReserves, initWith1Reserve, initWith2Reserves, initLPV2, initWithEtherReserve, initWithETHReserve],
		[...VERSIONS, null],
		[false, true]
	);
	const combinations = product.filter(
		([init, version, active]) => !(init === initWithoutReserves && active) && !(init === initWithETHReserve && version)
	);
	...
	for (const [init, version, activate] of combinations) {
		describe(`${init.name}(version = ${version || "latest"}, activate = ${activate}):`, () => {
			it("should upgrade successfully", async () => {
				...
				const upgradedReserveBalances = [activate ? RESERVE1_BALANCE : new BN(0), activate ? RESERVE2_BALANCE : new BN(0)];

				const stakedBalances = [activate ? RESERVE1_BALANCE : new BN(0), activate ? RESERVE2_BALANCE : new BN(0)];
