...
contract("LiquidityPoolV2Converter", (accounts) => {
	[
		...
		describe(`${decimalsDesc} ${reservesDesc}`, () => {
			const initConverter = async (activate, addLiquidity, maxConversionFee = 0, primaryReserveAddress = getReserve1Address(isETHReserve)) => {
			...
			const getExpectedTargetAmount = (
				...
				const targetAmount = new BN(
					crossReserveTargetAmount(sourceBalance, sourceWeight, targetBalance, targetWeight, amount).round().toString()
				);
			...
			const convertAndReturnTargetAmount = async (account, converter, sourceToken, sourceTokenAddress, targetTokenAddress, amount) => {
				...
				if (sourceTokenAddress === ETH_RESERVE_ADDRESS) {
					...
					await sourceToken.approve(sovrynSwapNetwork.address, amount, { from: account });
				}
			...
			describe("min-return", () => {
				it("addLiquidity should revert when min-return is zero", async () => {
					...
					await expectRevert(converter.addLiquidity(reserveToken2.address, amount, new BN(0), { from: sender }), "ERR_ZERO_VALUE");
				});

				it("removeLiquidity should revert when min-return is zero", async () => {
					const amount = toReserve2(new BN(100));
					const converter = await initConverter(true, true);
					await expectRevert(converter.removeLiquidity(poolToken2.address, amount, new BN(0), { from: sender }), "ERR_ZERO_VALUE");
				});
				...
				it("removeLiquidity should revert when min-return is larger than return", async () => {
					const converter = await initConverter(true, true);
					await expectRevert(
						converter.removeLiquidity(poolToken2.address, toReserve2(new BN(1)), toReserve2(new BN(100)), { from: sender }),
						"ERR_RETURN_TOO_LOW"
					);
				});
			});
			...
			it("should revert when attempting to activate an active converter", async () => {
				...
				await expectRevert(
					converter.activate(getReserve1Address(isETHReserve), chainlinkPriceOracleA.address, chainlinkPriceOracleB.address),
					"ERR_ACTIVE"
				);
			});
			...
			it("should revert when attempting to activate the converter with the converter address as the primary reserve oracle address", async () => {
				...
				await expectRevert(
					converter.activate(getReserve1Address(isETHReserve), converter.address, chainlinkPriceOracleB.address),
					"ERR_ADDRESS_IS_SELF"
				);
			});
			...
			it("should revert when attempting to activate the converter with the converter address as the secondary reserve oracle address", async () => {
				...
				await expectRevert(
					converter.activate(getReserve1Address(isETHReserve), chainlinkPriceOracleA.address, converter.address),
					"ERR_ADDRESS_IS_SELF"
				);
			});
			...
			it("should revert when attempting to update the dynamic-fee factor to an invalid value", async () => {
				...
				await expectRevert(converter.setDynamicFeeFactor(MAX_DYNAMIC_FEE_FACTOR.add(new BN(1))), "ERR_INVALID_DYNAMIC_FEE_FACTOR");
			});
			...
			for (const tokenAmount of [1000, 1000000, 1000000000]) {
				context(`converting ${tokenAmount} tokens`, async () => {
					...
					it("verifies balances after conversion", async () => {
						...
						const res = await convert([getReserve1Address(isETHReserve), anchorAddress, reserveToken2.address], amount, MIN_RETURN, {
							value,
						});
			...
			it("should revert when attempting to convert when the return is smaller than the minimum requested amount", async () => {
				...
				await expectRevert(
					convert([getReserve1Address(isETHReserve), anchorAddress, reserveToken2.address], amount, toReserve2(new BN(20000)), { value }),
					"ERR_RETURN_TOO_LOW"
				);
			});
			...
			it("should revert when attempting to buy while the purchase yields 0 return", async () => {
				...
				if (isETHReserve) {
					...
					await reserveToken.approve(sovrynSwapNetwork.address, amount, { from: sender });
					await reserveToken2.approve(sovrynSwapNetwork.address, amount, { from: sender });
				}

				if (reserveToken1Decimals.gte(reserveToken2Decimals)) {
					await expectRevert(
						convert([getReserve1Address(isETHReserve), anchorAddress, reserveToken2.address], amount, MIN_RETURN, { value }),
						"ERR_ZERO_TARGET_AMOUNT"
					);
				} else {
					await expectRevert(
						convert([reserveToken2.address, anchorAddress, getReserve1Address(isETHReserve)], amount, MIN_RETURN, { value }),
						"ERR_ZERO_TARGET_AMOUNT"
					);
				}
			});
			...
			it("should revert when attempting to convert when target reserve is invalid", async () => {
				...
				await expectRevert(
					convert([getReserve1Address(isETHReserve), anchorAddress, token.address], amount, MIN_RETURN, { value }),
					"ERR_INVALID_RESERVE"
				);
			});
			...
			if (!isETHReserve) {
				describe("exit-fee", () => {
					for (let i = 1; i <= 5; i++) {
						...
						it(`convert ${reserveTokenAmount.toString()} reserve tokens and then remove ${poolTokenAmount.toString()} pool tokens`, async () => {
							const converter = await initConverter(true, true);
							await reserveToken.approve(sovrynSwapNetwork.address, reserveTokenAmount, { from: sender });
							await convert([reserveToken.address, anchorAddress, reserveToken2.address], reserveTokenAmount, MIN_RETURN, {
								from: sender,
							});
				...
				describe("adding ETH liquidity", () => {
					it("should revert when attempting to add liquidity with ETH value when there is no ETH reserve", async () => {
						...
						await reserveToken.approve(converter.address, amount, { from: sender });
						await expectRevert(converter.addLiquidity(ETH_RESERVE_ADDRESS, amount, MIN_RETURN, { value: amount }), "ERR_INVALID_RESERVE");
					});
				});
			} else {
				describe("adding ETH liquidity", () => {
				...
				for (const isReserve1Primary of [true, false]) {
					describe(`${isReserve1Primary ? "reserve1 is primary" : "reserve2 is primary"}`, () => {
						...
						const getExpectedReferenceRate = (referenceRate, lastConversionRate, timeElapsed) => {
							...
							const newReferenceRateN = referenceRate.n
								.mul(lastConversionRate.d)
								.mul(RATE_PROPAGATION_PERIOD.sub(timeElapsed))
						...
						context("after external oracle update", async () => {
							...
							it("should continue calculating the rate after external oracle rate update", async () => {
								...
								const { referenceRate: expectedReferenceRate } = getExpectedReferenceRate(referenceRate, lastConversionRate, delta);
						...
						[
							...
							context(`with conversion after ${timeElapsed.toString()} seconds`, async () => {
								...
								it("should properly calculate the internal rate", async () => {
									...
									const { referenceRate: expectedReferenceRate } = getExpectedReferenceRate(
										normalizedRate,
										normalizedRate,
										timeElapsed
									);
