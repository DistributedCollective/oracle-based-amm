...
const OLD_CONVERTER_CONTRACT = process.argv[2];
const OLD_CONVERTER_ADDRESS = process.argv[3];
...
const upgradeConverterWithNewOracles = async (web3, upgrader, oldConverter, newConverterContract, newTokenAOracle, newTokenBOracle) => {
    await execute(oldConverter.methods.transferOwnership(upgrader._address));
    await execute(upgrader.methods.upgradeWithNewOracles(oldConverter._address, newTokenAOracle, newTokenBOracle, web3.utils.asciiToHex('')));
...
const testUpgradeConverter = async() => {
    ...
    let newConverter = await upgradeConverter(web3, upgrader, oldConverter, 'LiquidityPoolV2Converter');
...
const testUpgradeConverterWithNewOracles = async() => {
    let oldConverter = deployed(web3, OLD_CONVERTER_CONTRACT, OLD_CONVERTER_ADDRESS);
    ...
    let newConverter = await upgradeConverterWithNewOracles(web3, upgrader, oldConverter, 'LiquidityPoolV2Converter', oldConverterStateBefore.tokenBOracle, oldConverterStateBefore.tokenAOracle);
...
testUpgradeConverterWithNewOracles();
