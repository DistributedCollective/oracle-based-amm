    ...
    for (const converter of getConfig().converters) {
        ...
        if (type !== 0 && amounts.every(amount => amount > 0)) {
            for (let i = 0; i < converter.reserves.length; i++) {
                ...
                if (type == 2) {
                    if (!reserve.oracle) {
                        const oracleName = reserve.symbol === 'RBTC' ? 'MocBTCToUSDOracle' : 'MocBTCToBTCOracle'; 
                        const mocOracleArgs = oracleName === 'MocBTCToUSDOracle' ? [getData().mocMedianizerMockUSDtoBTC.addr] : [];
