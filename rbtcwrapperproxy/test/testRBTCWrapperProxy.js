...
contract("RBTCWrapperProxy", async (accounts) => {
	...
	it("verifies that users could send RBTC and SOV, and then add liquidity to get pool token 1", async () => {
		...
		expectedBalance = new BN(sovAmountAfter.toString()).sub(web3.utils.toBN(sovAmount * 2));
	...
	it("verifies that users could send RBTC and then swap it to DoC", async () => {
		...
		var result = await rbtcWrapperProxy.convertByPath(pathWRBTCToDoC, web3.utils.toBN(1e16), constants.ZERO_ADDRESS, 1, {
			from: accounts[0],
			to: RBTCWrapperProxy.address,
			value: 1e16
		});
	...
	it("verifies that users could send DoC and then swap it to RBTC", async () => {
		...
		var result = await rbtcWrapperProxy.convertByPath(pathDoCToWRBTC, web3.utils.toBN(1e20), constants.ZERO_ADDRESS, 1, {
			from: accounts[0],
			to: RBTCWrapperProxy.address,
		});
	...
	it("should revert when passing wrong path param to convertByPath()", async () => {
		...
		var pathDoCToWRBTC = await sovrynSwapNetwork.conversionPath(usdTokenAddress, wrbtcAddress);
		await expectRevert.unspecified(
			rbtcWrapperProxy.convertByPath(pathDoCToWRBTC, web3.utils.toBN(1e16), constants.ZERO_ADDRESS, 1, {
				from: accounts[0],
				to: RBTCWrapperProxy.address,
				value: 1e16,
			}),
		...
		await expectRevert.unspecified(
			rbtcWrapperProxy.convertByPath(pathWRBTCToDoC, web3.utils.toBN(1e20), constants.ZERO_ADDRESS, 1, { from: accounts[0], to: RBTCWrapperProxy.address }),
			"Wrong path param"
		);
	});

	it("Should revert when providing wrbtc address in second index reserveTokens", async() => {
			...
			1, 
		{
			...
			to: RBTCWrapperProxy.address,
			value: rbtcAmount
		}), "WRBTC must be in the 0 index");
	})

	it("Should revert rbtc sent not the same as the value params", async() => {
		const rbtcAmount = web3.utils.toBN(getConfigFromSOV()["converters"][0]["reserves"][0]["balance"] * 1e14);
		const sovAmount = web3.utils.toBN(getConfigFromSOV()["converters"][0]["reserves"][1]["balance"] * 1e14);
		await expectRevert(rbtcWrapperProxy.addLiquidityToV1(
			liquidityPoolV1ConverterAddress, 
	...
	it("Should revert if sending > 0 as amount params for non-wrbtc pair", async() => {
		const susdAmount = web3.utils.toBN(getConfigFromSOV()["converters"][0]["reserves"][0]["balance"] * 1e14);
		const sovAmount = web3.utils.toBN(getConfigFromSOV()["converters"][0]["reserves"][1]["balance"] * 1e14);
		await expectRevert(rbtcWrapperProxy.addLiquidityToV1(
			liquidityPoolV1ConverterAddress, 
			[usdTokenAddress, sovTokenAddress],
			[susdAmount, sovAmount],
			1, 
		{
			from: accounts[0],
			to: RBTCWrapperProxy.address,
			value: 1
		}), "0 amount required for non-wrbtc pairs");
	})
	...
	it("verifies that users could send USDT and SOV, and then add liquidity to get pool token 1", async () => {
		var sovAmountBefore = await sovToken.balanceOf(accounts[0]);
		var usdAmountBefore = await usdToken.balanceOf(accounts[0]);
		...
		let usdRate = await sovrynSwapNetwork.rateByPath(pathWRBTCToDoC, rbtcAmount);

		await sovToken.approve(RBTCWrapperProxy.address, web3.utils.toBN(0), { from: accounts[0] });
		await sovToken.approve(RBTCWrapperProxy.address, web3.utils.toBN(sovAmount * 4), { from: accounts[0] });

		let userInitialBtcBalance = new BN(await web3.eth.getBalance(accounts[0]));
		var result = await rbtcWrapperProxy.addLiquidityToV1(
		...
		let decode = decodeLogs(result.receipt.rawLogs, WRBTC, "Withdrawal");
		const refundedRBTC = decode[0].args.wad;

		const gasPrice = (await web3.eth.getTransaction(result.tx)).gasPrice;
		let txFee = new BN(result.receipt.gasUsed).mul(new BN(gasPrice));
		let userLatestBTCBalance = new BN(await web3.eth.getBalance(accounts[0]));

		expect(userLatestBTCBalance.toString()).to.equal( (userInitialBtcBalance.sub(rbtcAmount).sub(txFee).add(new BN(refundedRBTC))).toString() )

		var addedPoolTokenV1Amount = new BN(result.logs[2].args._poolTokenAmount);
		var expectedBalance = new BN(sovAmountBefore.toString()).sub(sovAmount)
		assert.equal(
			(await sovToken.balanceOf(accounts[0])).toString(),
			expectedBalance.toString(),
		...
		await expectEvent(result.receipt, "LiquidityAddedToV1", {
			...
			_reserveAmounts: [usdRate, sovAmount], 
			_poolTokenAmount: addedPoolTokenV1Amount,
		});

		var sovAmountAfter = await sovToken.balanceOf(accounts[0]);
		usdRate = await sovrynSwapNetwork.rateByPath(pathWRBTCToDoC, rbtcAmount);

		// Send 2x SOV, User should get 1x SOV back
		var result = await rbtcWrapperProxy.addLiquidityToV1(
			liquidityPoolV1ConverterUSDSOV, 
			[usdTokenAddress,sovTokenAddress], 
			[0, web3.utils.toBN(sovAmount * 2)], 
			2, 
		{
			from: accounts[0],
			to: RBTCWrapperProxy.address,
		...
		addedPoolTokenV1Amount = addedPoolTokenV1Amount.add(new BN(result.logs[2].args._poolTokenAmount));
		expectedBalance = new BN(sovAmountAfter.toString()).sub(web3.utils.toBN(sovAmount * 2));
		...
		await expectEvent(result.receipt, "LiquidityAddedToV1", {
			_provider: accounts[0],
			_reserveTokens: [usdTokenAddress, sovTokenAddress],
			_reserveAmounts: [usdRate, web3.utils.toBN(sovAmount * 2)]
		});
	});
